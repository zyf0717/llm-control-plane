...
COMPACTION_MAX_ANCHORS = 30
COMPACTION_MANDATORY_ANCHORS = 12
COMPACTION_MAX_EVIDENCE_SNIPPETS = 12

_COMPACTION_PROMPT_OVERHEAD_CHARS = 12_000
_COMPACTION_SNIPPET_CHARS = 700


def parse_compaction_input(
...
def chunk_compaction_units(units: list[str], budget: int) -> list[str]:
    chunks: list[str] = []
    current: list[str] = []
    current_len = 0
    for unit in units:
...
def chunk_text_by_budget(text: str, budget: int) -> list[str]:
    ...
    chunks = chunk_compaction_units(units, budget)
    if chunks:
        return chunks
    return [
        source[index : index + budget]
...
def check_compaction_input_payload(
...
def build_compaction_prompt(
    *,
    ...
    anchors_json = json.dumps(
        anchors[:COMPACTION_MAX_ANCHORS],
        ensure_ascii=False,
        indent=2,
    )
    evidence_json = json.dumps(
        evidence[:COMPACTION_MAX_EVIDENCE_SNIPPETS],
        ensure_ascii=False,
        indent=2,
    )
    return (
...
def build_compaction_repair_prompt(
    *,
    text: str,
    error: str,
    source_text: str,
    ...
    source_excerpt = compaction_source_excerpt(source_text, step)
    contract = step.output_contract
    format_instructions = (
    ...
    anchors_json = json.dumps(
        anchors[:COMPACTION_MANDATORY_ANCHORS],
        ensure_ascii=False,
        indent=2,
    )
    return (
...
def compaction_source_excerpt(source_text: str, step: WorkflowStepSpec) -> str:
...
def validate_compaction_output_text(
    ...
    missing_anchors = missing_mandatory_anchors(compacted, anchors)
    if missing_anchors:
        return {
            "valid": False,
            "error": (
...
def compaction_needs_reduce(summaries: list[str], step: WorkflowStepSpec) -> bool:
...
def compaction_output_payload(
    *,
    ...
    metadata_json: dict[str, Any] = {
        ...
        "rounds": rounds,
        "method": method,
        "preserved_keywords": anchors[:COMPACTION_MAX_ANCHORS],
    ...
    set_compaction_output_json_bytes(output)
    return output


def set_compaction_output_json_bytes(output: dict[str, Any]) -> None:
...
def check_compaction_output_payload(
...
def extract_compaction_anchors(text: str) -> list[str]:
    source = str(text or "")
    candidates: list[str] = []
    ...
    for candidate in candidates:
        ...
        if len(anchors) >= COMPACTION_MAX_ANCHORS:
            break
    return anchors


def top_compaction_evidence(
    ...
    for rank, (score, negative_index, snippet) in enumerate(
        scored[:COMPACTION_MAX_EVIDENCE_SNIPPETS],
        start=1,
    ):
        evidence.append(
            {
